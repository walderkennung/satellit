...
- `satellit -- predict image-masks --image <image>` is the canonical strong-label workflow and writes:
  - run metadata at `metadata.json` in the output directory root (`tile,image_path,prompt,model,command`)
...
- `satellit -- label validate-predictions --image-tif <image.tif> --inventory-{csv|shp} <inventory> --predictions-npz <masks/image_masks.npz>` validates from merged predictions.
- `satellit -- label validate-predictions --image-tif <image.tif> --inventory-{csv|shp} <inventory> --predictions-tiles-dir <masks/tiles>` validates from per-tile predictions and supports interrupted runs (discovers `tile_x*_y*.npz`; `index.csv` is optional).
- `label validate-predictions` output includes `tree_id,stem_id,label_id,tree_pos_x,tree_pos_y,prediction_coverage`.
